package com.kyovo.hackerrank.practice.problemsolving.datastrutctures.arrays.easy.twodarrayds;

public class ArrayDs {
    private static final int HOURGLASS_MIN_VALUE = -9;
    private static final int MAX_ELEMENTS_IN_HOURGLASS = 7;

    public static int hourglassSum(int[][] arr) {
        int rows = arr.length;
        int columns = arr[0].length;
        int maxHourglassSum = HOURGLASS_MIN_VALUE * MAX_ELEMENTS_IN_HOURGLASS;

        for (int i = 0; i < rows-2; i++) {
            for (int j = 0; j < columns-2; j++) {
                int currentHourglassSum = arr[i][j] + arr[i][j+1] + arr[i][j+2]
                                                    + arr[i+1][j+1]
                                        + arr[i+2][j] + arr[i+2][j+1] + arr[i+2][j+2];

                maxHourglassSum = Math.max(maxHourglassSum, currentHourglassSum);
            }
        }

        return maxHourglassSum;
    }
}
